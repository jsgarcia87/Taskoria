...
const StatusBadge = ({ status }) => {
    if (status === 'approved') return <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold text-green-400"><CheckCircle2 size={10}/> Aprobado</span>;
    if (status === 'rejected') return <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold text-red-400"><XCircle size={10}/> Rechazado</span>;
    return <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold text-amber-400"><Clock size={10}/> Pendiente</span>;
};
    ...
    return (
        <div className="text-white">
            <header className="mb-6 text-center">
                <h1 className="text-3xl font-display font-bold tracking-wide text-rpg-gold">Creaciones de Taskoria</h1>
                <p className="text-sm text-gray-400">Lo que la comunidad está construyendo para el mundo.</p>
            </header>

            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div className="flex gap-2">
                    <button onClick={() => setTab('public')} className={`text-xs uppercase tracking-widest px-3 py-1.5 rounded border ${tab==='public' ? 'bg-rpg-gold/20 border-rpg-gold text-rpg-gold' : 'bg-white/5 hover:bg-white/10 border-white/10'}`}>Galería pública</button>
                    {currentUser?.id && (
                        <button onClick={() => setTab('mine')} className={`text-xs uppercase tracking-widest px-3 py-1.5 rounded border ${tab==='mine' ? 'bg-rpg-gold/20 border-rpg-gold text-rpg-gold' : 'bg-white/5 hover:bg-white/10 border-white/10'}`}>Mis creaciones</button>
                    )}
                </div>
            ...
            {loading ? (
                <div className="flex items-center justify-center py-20 text-gray-400"><Loader2 className="animate-spin mr-2" size={18}/> Cargando...</div>
            ) : items.length === 0 ? (
                <div className="text-center py-20 text-gray-500 text-sm">
                    {tab === 'public' ? 'Aún no hay creaciones aprobadas en esta categoría.' : 'No has publicado nada todavía.'}
                </div>
            ) : (
                    ...
                    {items.map(item => (
                        <div key={item.id} className="glass-panel rounded-xl p-3 flex flex-col items-center text-center">
                            ...
                            {item.username && tab === 'public' && (
                                <div className="text-[10px] text-gray-400 mt-1">por {item.username}</div>
                            )}
                            {tab === 'mine' && (
